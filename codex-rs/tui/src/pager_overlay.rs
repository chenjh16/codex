...
struct PagerView {
    ...
    /// If set, on next render ensure this target is visible.
    pending_scroll_target: Option<ScrollTarget>,
}
...
impl PagerView {
    ...
    fn render(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        if let Some(target) = self.pending_scroll_target.take() {
            self.ensure_target_visible(target, content_area);
        }
...
impl PagerView {
    ...
    fn scroll_chunk_into_view(&mut self, chunk_index: usize) {
        self.pending_scroll_target = Some(ScrollTarget::Chunk(chunk_index));
    }

    fn scroll_line_into_view(&mut self, chunk_index: usize, line_index: usize) {
        self.pending_scroll_target = Some(ScrollTarget::Line {
            chunk_index,
            line_index,
        });
    }

    fn ensure_target_visible(&mut self, target: ScrollTarget, area: Rect) {
        match target {
            ScrollTarget::Chunk(chunk_index) => self.ensure_chunk_visible(chunk_index, area),
            ScrollTarget::Line {
                chunk_index,
                line_index,
...
impl TranscriptOverlay {
    ...
    pub(crate) fn render(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        if let Some(target) = self.view.pending_scroll_target.take() {
            self.view.ensure_target_visible(target, content_area);
        }
